...
class SPPNET:
    def __init__(self):
        ...
        self.image_holder=tf.placeholder(shape=[None,24,24,3],dtype=tf.float32,name="images")                          
        self.label_holder=tf.placeholder(shape=[None],dtype=tf.int64,name="labels")
    ...
    def inference(self,x):
        ...
        ###SPPnet优化
        poo2_out_map_width=pool2_out.get_shape()[1].value
        spp_pool1_width=int(poo2_out_map_width/3)
        spp_pool2_width=int(poo2_out_map_width/2)
        spp_pool3_width=int(poo2_out_map_width/1)
        
        spp_pool1=self.maxpool(pool2_out,spp_pool1_width)
        spp_pool2=self.maxpool(pool2_out,spp_pool2_width)
        spp_pool3=self.maxpool(pool2_out,spp_pool3_width)

        flatten_pool1=tf.reshape(spp_pool1,[-1,3*3*64])
        flatten_pool2=tf.reshape(spp_pool2,[-1,2*2*64])
        flatten_pool3=tf.reshape(spp_pool3,[-1,1*1*64])
        spp_out=tf.concat([flatten_pool1,flatten_pool2,flatten_pool3],1)
        #flatten=tf.reshape(pool2_out,[batchsize,-1])
        ####fc1
        fc_1_dict=network_options["net"]["fc"][0]
        #flatten=tf.reshape(pool2_out,[-1,6*6*64])
        dim=spp_out.get_shape()[1].value
        weights_fc1=self.weight_init([dim,fc_1_dict["hidden_dim"]],0.004,'fc1_w')
        bias_fc1=self.bias_init([fc_1_dict["hidden_dim"]],'fc1_b')
        fc1_out=tf.nn.relu(tf.matmul(spp_out,weights_fc1)+bias_fc1)
        ###fc2
        ...
        output=tf.add(tf.matmul(fc1_out,weights_fc2),bias_fc2,name="output")
        return output
        ...
        with tf.Session(graph=tf.get_default_graph(),config=config) as sess:
            ...
            for epoch in range(n_epoch):
                ...
                for i in range(self.max_steps):
                    ...
                    print("Iteration %d,train loss is %.4f" % (i,batch_loss))
            ...
            with tf.gfile.FastGFile(model_path+"spp_model.pb","wb") as fo:
